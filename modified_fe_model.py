...
def simulateParticle(m_bar, scale_height, isothermal_temp, p_sur, c_sp, m_mol,
    ...
    Inputs:
        ...
        isothermal_temp - the isothermal atmospheric temperature [K]
        p_sur - the atmospheric surface pressure [Pa]
        c_sp - specific heat of the micrometeorite [units?????????????????????]
        m_mol - molar mass of FeO [g mol-1]
        L_v - latent heat of vaporization of FeO [units???????????????????????]
        rad - micrometeorite radius [m]
        dt - simulation timesteap [s]
        v_0 - initial micrometeorite velocity [km s-1]
        theta - initial micrometeorite entry angle (between 0 and pi/2) [rad]

    Returns:
        altitudes  - altitude points of micrometeorite [m]
        phis       - micrometeorite angle above Earth surface [rad]
        temps      - micrometeorite temperature [K]
        rads       - micrometeorite radius [m]
        velocities - micrometeorite velocity [m s-1]
        times      - model time [s]
    """
...
#            rads[0:end_index+1], altitudes[0:end_index+1], times[0:end_index+1])
    ...
    return altitudes[0:end_index+1], \
           phis[0:end_index+1], \
           temps[0:end_index+1], \
           rads[0:end_index+1], \
           velocities[0:end_index+1], \
           times[0:end_index+1]
...
def runModel():
    ...
    count = 10
    input_rads = np.linspace(50*1.0E-6, 500*1.0E-6, count)
...
def plotRadiusEntryTemp(rads, thetas, temps):
    """
    Plot the contours of radius vs entry angle and the resulting temperature.
    This reproduces figure 4 of Genge (2016).
    """

    levels = [1800,1900,2000,2100,2200,2300,2400,2500,2600,2700,2800,2900,3000]
    CS = plt.contour(rads/(1.0E-6), thetas/pi*180, temps, levels)
    #plt.imshow(temps, origin="lower", cmap="cool", interpolation="nearest")
